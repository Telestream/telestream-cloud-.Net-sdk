...
namespace Telestream.Cloud.SDK
{
        public class FactoryService : ServiceBase
        {
                ...
                public Task<UploadSession> BeginUpload(string fileName, Stream dataStream, CancellationToken cancelToken = default(CancellationToken),
                                                       string storeId = null)
                {
                        return BeginUpload(fileName, dataStream, null, null, null, cancelToken, storeId);
                }
                ...
                public Task<UploadSession> BeginUpload(string fileName, Stream dataStream, string pathFormat,
                                                       ...
                                                       string storeId = null)
                {
                        return _cloudService.StartUploadSession(FactoryId, dataStream.Length, fileName, pathFormat, extraVars, profiles, cancelToken, storeId);
                }

                public async Task<Video> UploadFile(string fileName, Stream dataStream, string pathFormat = null,
                                                    Dictionary<string, string> extraVars = null, string profiles = null,
                                                    IProgress<double> progress = null,
                                                    CancellationToken cancelToken = default(CancellationToken),
                                                    string storeId = null)
                {
                        var session = await BeginUpload(fileName, dataStream, pathFormat, extraVars, profiles, cancelToken, storeId);
