...
namespace Telestream.Cloud.SDK
{
        public class TelestreamCloudService : ServiceBase
        {
            ...
            public Task<Video> CreateVideo(string factoryId, Video video)
            {
                ...
                public Task<UploadSession> StartUpload(string factoryId, long fileSize, string fileName, CancellationToken cancelToken = default(CancellationToken), string storeId = null)
                {
                        return StartUploadSession(factoryId, fileSize, fileName, null, null, null, cancelToken, storeId);
                }
